...
def find_centroid(cluster):
    ...
    lats, lons = zip(*locations)    # make all latitude a list and all longitude another list
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        cluseters = group_by_centroid(restaurants, centroids)
        for i in range(len(cluseters)):
            centroids[i] = find_centroid(cluseters[i])
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared = Sxy*1.0/Sxx, mean_y-mean_x*Sxy*1.0/Sxx, Sxy**2/(Sxx*Syy) 
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    """
    reviewed = user_reviewed_restaurants(user, restaurants)   # reviewed is a restautants list
    # BEGIN Question 8
    predictors = [find_predictor(user, reviewed, f) for f in feature_fns]
    return max(predictors, key=lambda prs: prs[1])[0]
    # END Question 8
