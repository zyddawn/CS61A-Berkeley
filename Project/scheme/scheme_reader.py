...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            second = scheme_read(src)							# get the first value/expr after '.'
            if src.remove_front() == ")":						# check if there're any other values
                return second									# return the first value
            # LATENT BUG & INCOMPLETE (cannot detect syntax error like "read_line("(1 2 . 3) (")")
