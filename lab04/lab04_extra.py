...
def flatten(lst):
    ...
    """
    flat = []
    def flat_list(prev_list, sublist):
        for i in sublist:
            if isinstance(i, list):
                prev_list = flat_list(prev_list, i)
            else:
                prev_list.append(i)
        return prev_list
    return flat_list(flat, lst)
...
def merge(lst1, lst2):
    ...
    merge_list = []
    id_1 = id_2 = 0
    while id_1<len(lst1)-1 and id_2<len(lst2)-1:
        if lst1[id_1] < lst2[id_2]:
            merge_list.append(lst1[id_1])
            id_1 += 1
        else:
            merge_list.append(lst2[id_2])
            id_2 += 1
    if id_1<len(lst1)-1:
        merge_list += lst1[id_1:]
    if id_2<len(lst2)-1:
        merge_list += lst2[id_2:]
    return merge_list
...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows):
        cur_row = max_rows-1-i
        if board[cur_row][column] == '-':
            board[cur_row] = replace_elem(board[cur_row], column, player)
